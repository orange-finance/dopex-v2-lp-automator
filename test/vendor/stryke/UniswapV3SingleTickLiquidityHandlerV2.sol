...
pragma solidity <0.9.0 =0.8.15 >=0.4.0 >=0.5.0 >=0.6.0 >=0.7.5 >=0.8.0 ^0.8.0 ^0.8.1 ^0.8.13;
...
// src/libraries/tokens/ERC6909.sol
...
abstract contract LiquidityManager is IUniswapV3MintCallback {
    ...
    ) public returns (uint128 liquidity, uint256 amount0, uint256 amount1, IUniswapV3Pool pool) {
...
// src/handlers/UniswapV3SingleTickLiquidityHandlerV2.sol
 ...
 */
contract UniswapV3SingleTickLiquidityHandlerV2 is ERC6909, IHandler, Pausable, AccessControl, LiquidityManager {
    ...
    function mintPositionHandler(
        ...
        if (posCache.amount0 > 0 && posCache.amount1 > 0) revert UniswapV3SingleTickLiquidityHandlerV2__InRangeLP();
        ...
        if (tki.totalSupply > 0) {
            ...
            if (tki.tokensOwed0 > 1_000 || tki.tokensOwed1 > 1_000) {
                uint256 expectedAmountForLiquidity0 = LiquidityAmounts.getAmount0ForLiquidity(
                    posCache.sqrtRatioTickLower,
                    posCache.sqrtRatioTickUpper,
                    2
                );

                uint256 expectedAmountForLiquidity1 = LiquidityAmounts.getAmount1ForLiquidity(
                    posCache.sqrtRatioTickLower,
                    posCache.sqrtRatioTickUpper,
                    2
                );
                ...
                if (expectedAmountForLiquidity0 > tki.tokensOwed0 || expectedAmountForLiquidity1 > tki.tokensOwed1) {
                    ...
                    (uint128 liquidityFee, , , ) = UniswapV3SingleTickLiquidityHandlerV2(address(this)).addLiquidity(
                        LiquidityManager.AddLiquidityParams({
                            token0: tki.token0,
                            token1: tki.token1,
                            fee: tki.fee,
    ...
    function usePositionHandler(
    ...
    ) external whenNotPaused returns (address[] memory, uint256[] memory, uint256) {
        onlyWhitelisted();
        ...
        if (_params.hook != address(0)) IHook(_params.hook).onPositionUse(hookData);
    ...
    function unusePositionHandler(
        ...
        (UnusePositionParams memory _params, bytes memory hookData) = abi.decode(
            _unusePositionData,
            (UnusePositionParams, bytes)
        );
        ...
        if (_params.hook != address(0)) IHook(_params.hook).onPositionUnUse(hookData);
     ...
     */
    function getHandlerIdentifier(bytes calldata _data) external view returns (uint256 handlerIdentifierId) {
        (IUniswapV3Pool pool, address hook, int24 tickLower, int24 tickUpper) = abi.decode(
            _data,
            (IUniswapV3Pool, address, int24, int24)
        );
    ...
    function onlyWhitelisted() private {
        if (!whitelistedApps[msg.sender]) revert UniswapV3SingleTickLiquidityHandlerV2__NotWhitelisted();
    }
